...
#[test]
fn build_workspaces() {
    let (cdir, ddir) = extract_built_package_from_manifest(
        "tests/test-workspace/test-ws1/Cargo.toml",
        &["--no-strip", "--fast"],
    );
    ...
    let (_, ddir) = extract_built_package_from_manifest(
        "tests/test-workspace/test-ws2/Cargo.toml",
        &["--no-strip"],
    );
    assert!(ddir.path().join("usr/bin/renamed2").exists());
    assert!(ddir
        .path()
        .join(format!("usr/lib/{DLL_PREFIX}test2lib{DLL_SUFFIX}"))
        .exists());
}
...
fn extract_built_package_from_manifest(manifest_path: &str, args: &[&str]) -> (TempDir, TempDir) {
    ...
    let last_line = output.stdout[..output.stdout.len() - 1]
        .split(|&c| c == b'\n')
        .last()
        .unwrap();
...
fn run_cargo_deb_command_on_example_dir() {
    ...
    assert!(ddir
        .path()
        .join("usr/share/doc/example/changelog.Debian.gz")
        .exists());
...
fn run_cargo_deb_command_on_example_dir_with_variant() {
    ...
    let control = fs::read_to_string(cdir.path().join("control")).unwrap();
    assert!(
        control.contains("Package: example-debug\n"),
        "Control is: {:?}",
        control
    );
    assert!(control.contains("Version: 0.1.0\n"));
    ...
    assert!(md5sums
        .contains("2455967cef930e647146a8c762199ed3  usr/share/doc/example-debug/copyright\n"));
    ...
    assert!(ddir
        .path()
        .join("usr/share/doc/example-debug/copyright")
        .exists());
    assert!(ddir
        .path()
        .join("usr/share/doc/example-debug/changelog.Debian.gz")
        .exists());
    assert!(ddir.path().join("usr/bin/example").exists());
}
